...
io.on("connection", (socket) => {
        ...
        socket.on('firstKey', async (message) => {
            ...
            io.sockets.emit('testKey', "Message ->"+message);
        });
            ...
            if(((receiverSocketId!='')||(receiverSocketId!=null))&&(sendFirebaseNotification==0))
            {
                ...
                if((sendFirebaseNotification==1)&&(message!=''))
                {
                    if(isNotification==1)
                    {
                        ...
                        {
                            sendAndroidNotify(receiverDeviceToken,message,from_id,receiverUserName);    
                        }
                        
                        //ios
                        if(receiverDeviceType==2)
                        {
                            sendIosNotify(receiverDeviceToken,message,from_id,receiverUserName);    
                        }
                    }
